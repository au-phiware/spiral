...
var _setToStringTag = function _setToStringTag(it, tag, stat) {
  if (it && !has$2(it = stat ? it : it.prototype, TAG)) def$1(it, TAG, { configurable: true, value: tag });
};
...
var _objectKeysInternal = function _objectKeysInternal(object, names) {
  var O = toIObject$2(object),
      ...
      key;
  for (key in O) {
    if (key != IE_PROTO) has$3(O, key) && result.push(key);
...
'hasInstance,isConcatSpreadable,iterator,match,replace,search,species,split,toPrimitive,toStringTag,unscopables'.split(','), i$1 = 0; symbols.length > i$1;) {
  wks(symbols[i$1++]);
}for (var symbols = $keys(wks.store), i$1 = 0; symbols.length > i$1;) {
...
var construct$1 = function construct$1(F, len, args) {
  if (!(len in factories)) {
...
var isInteger$2 = _isInteger;
var abs$1 = Math.abs;

$export$19($export$19.S, 'Number', {
  isSafeInteger: function isSafeInteger(number) {
    return isInteger$2(number) && abs$1(number) <= 0x1fffffffffffff;
  }
});
...
&& $acosh(Infinity) == Infinity), 'Math', {
  acosh: function acosh(x) {
    return (x = +x) < 1 ? NaN : x > 94906265.62425156 ? Math.log(x) + Math.LN2 : log1p$1(x - 1 + sqrt$1(x - 1) * sqrt$1(x + 1));
  }
});
...
if (_descriptors && (!CORRECT_NEW || _fails(function () {
  ...
  for (var keys$2 = gOPN$3(Base$1), i$2 = 0; keys$2.length > i$2;) {
    proxy(keys$2[i$2++]);
...
var empty$1 = function empty$1() {/* empty */};
var Internal;
...
while (i$3 < l) {
  if (Typed = global$12[TypedArrayConstructors[i$3++]]) {
...
function get$1(target, propertyKey /*, receiver*/) {
  ...
  if (desc = gOPD$4.f(target, propertyKey)) return has$9(desc, 'value') ? desc.value : desc.get !== undefined ? desc.get.call(receiver) : undefined;
...
var wrap$1 = function wrap$1(set) {
  return MSIE ? function (fn, time /*, ...args */) {
    return set(invoke$3(partial, [].slice.call(arguments, 2), typeof fn == 'function' ? fn : Function(fn)), time);
  } : set;
};
...
for (var collections = ['NodeList', 'DOMTokenList', 'MediaList', 'StyleSheetList', 'CSSRuleList'], i$4 = 0; i$4 < 5; i$4++) {
...
var index$3 = function load(src, async) {
...
var proto$4 = map$1.prototype;

Set$2.prototype = set$2.prototype = {
  constructor: Set$2,
  has: proto$4.has,
  add: function add(value) {
    value += "";
...
var primes = new Promise(function (done, fail) {
  return request('a000040.txt').mimeType('text/plain').response(function (xhr) {
    return dsv(" ").parseRows(xhr.responseText, function (d) {
      return +d[1];
    });
  }).on('error', fail).get(done);
});

function isPrime(n) {
  return primes.then(function (primes) {
    return index$4(primes, n) >= 0;
  });
}
...
var PI = Math.PI;
var log10$1 = Math.log10;
var log2$1 = Math.log2;
var sqrt = Math.sqrt;
...
var squared = function squared(n) {
  var root = sqrt(n);
  var upper = Math.pow(ceil(root), 2);
  var lower = Math.pow(floor(root), 2);
  if (upper - lower === 0) return root;
  return floor(root) + (n - lower) / (upper - lower);
};
var parameter = function parameter(n) {
  return TAU * (options.step === 0 ? squared(n) : n * options.step / 2);
};
var radius = function radius(t) {
  return Math.pow(t, 1 / options.windingNumber) * Math.pow(10, options.scale);
};
var angle = function angle(t) {
  var a = Math.pow(options.sect, 2);
  if (isInteger$1(t / TAU, 128)) return a * PI / (a + 1);
  return (t % TAU + a * PI) / (a + 1);
};
var x = function x(a, r) {
  return r * cos(a);
};
...
var exit = function exit($) {
  return $.classed('hide', true);
};
var update = function update($) {
  return $.classed('hide', false).attr('cx', function (c) {
    return c.x;
  }).attr('cy', function (c) {
    return c.y;
  });
...
function loop() {
  ...
  var limit = performance.now() + options.frameDuration;
  var sample = [];
  while (performance.now() < limit) {
    ...
    i++;
    if (i > 1000000 || hypot$1(d.x, d.y) > options.viewBox.hypot) limit = 0;
  }
  var points = svg.selectAll('circle').data(sample, function (d) {
    return d.n;
  });
  points.enter().call(enter);
  points.call(update);
  if (limit) timer = requestAnimationFrame(loop);else {
...
select('.controls').selectAll('input').on('change checked', function () {
  if (this.name in options) {
    ...
    if (name === 'step') {
      ...
      select('#scale').node().value = options[name] = log10$1(+this.value);
    } else {
...
function isInteger$1(j) {
  var tolerance = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 8;

  var diff = abs(round(j) - j);
...
index$3('//s7.addthis.com/js/300/addthis_widget.js#domready=1');

window.options = options;
window.app = svg;
window.loop = loop;
window.onhashchange = onhashchange;
//# sourceMappingURL=archimedean-spiral.mjs.map
